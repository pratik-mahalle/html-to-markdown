...
describe('real-world', () => {
  it('real_world_blog_post: Blog post with headings, paragraphs, code blocks, and links converts to readable Markdown', () => {
    ...
    expect(content).toContain('# Getting Started with Rust');
    expect(content).toContain('## Installation');
    expect(content).toContain('## Hello World');
    expect(content).toContain('## Key Concepts');
    expect(content).toContain('cargo run');
    expect(content).toContain('[Mozilla](https://www.mozilla.org)');
    expect(content).toContain('- Ownership and borrowing');
...
assert_eq!(markdown, "# Hello");</code></pre><h2>ConversionOptions</h2><p>Configure conversion behavior using the builder pattern:</p><pre><code class="language-rust">let options = ConversionOptions::builder()
    ...
    expect(content).toContain('## convert_html');
    expect(content).toContain('### Parameters');
    expect(content).toContain('### Returns');
    expect(content).toContain('### Example');
    expect(content).toContain('## ConversionOptions');
    expect(content).toContain('> ');
    expect(content).toContain('thread-safe');
    expect(content).toContain('convert_html');
    expect(content).toContain('ConversionOptions');

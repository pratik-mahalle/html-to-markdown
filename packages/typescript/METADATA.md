...
```typescript
// Check if metadata functions are available
import { convertWithMetadata } from 'html-to-markdown';

if (typeof convertWithMetadata === 'function') {
...
The metadata extraction feature is **enabled by default** in the published npm package.
If building from source, the feature is enabled via the `default` feature flag in Cargo.toml.

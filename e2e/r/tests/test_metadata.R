...
test_that("structured_data_json_ld: JSON-LD script tag is stripped from output (security) but metadata may be extracted", {
  ...
  expect_true(grepl("My Article", content, fixed = TRUE))
  # content_not_contains
  expect_false(grepl("application/ld+json", content, fixed = TRUE))
  expect_false(grepl("@context", content, fixed = TRUE))
  expect_false(grepl("schema.org", content, fixed = TRUE))
})
...
test_that("structured_data_multiple_json_ld: Multiple JSON-LD blocks are all stripped from output", {
  ...
  expect_true(nchar(trimws(content)) > 0)
  # content_contains_all
  expect_true(grepl("Widget", content, fixed = TRUE))
  # content_not_contains
  expect_false(grepl("@context", content, fixed = TRUE))
  expect_false(grepl("BreadcrumbList", content, fixed = TRUE))
  expect_false(grepl("ListItem", content, fixed = TRUE))
})

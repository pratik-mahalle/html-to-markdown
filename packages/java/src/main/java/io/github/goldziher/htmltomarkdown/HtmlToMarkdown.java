...
public final class HtmlToMarkdown {
    ...
    public static String convert(final String html) {
        ...
        try (Arena arena = Arena.ofConfined()) {
            MemorySegment htmlSegment = HtmlToMarkdownFFI.toCString(
                arena, html
            );

            MemorySegment resultSegment =
                (MemorySegment) HtmlToMarkdownFFI.html_to_markdown_convert
                    .invoke(htmlSegment);
            ...
            if (resultSegment == null || resultSegment.address() == 0) {
                ...
                throw new ConversionException(
                    error != null ? error
                        : "Conversion failed with unknown error"
                );
            }
            ...
            try {
                ...
                HtmlToMarkdownFFI.html_to_markdown_free_string
                    .invoke(resultSegment);
            }
    ...
    public static String getVersion() {
        try {
            MemorySegment versionSegment =
                (MemorySegment) HtmlToMarkdownFFI
                    .html_to_markdown_version.invoke();
     ...
     * @param frequency sampling frequency in Hz (defaults to 1000 when
     *     non-positive)
     */
    public static void startProfiling(final String outputPath,
            final int frequency) {
        if (outputPath == null || outputPath.isBlank()) {
            throw new IllegalArgumentException("outputPath is required");
        }
        int freq = frequency > 0 ? frequency
            : DEFAULT_PROFILING_FREQUENCY;

        try (Arena arena = Arena.ofConfined()) {
            MemorySegment outputSegment =
                HtmlToMarkdownFFI.toCString(arena, outputPath);
            boolean ok = (boolean) HtmlToMarkdownFFI
    ...
    public static void stopProfiling() {
        try {
            boolean ok = (boolean) HtmlToMarkdownFFI
                .html_to_markdown_profile_stop.invoke();
            if (!ok) {
                String error = getLastError();
                throw new ConversionException(
     ...
     * <p>
     * This method converts HTML to Markdown while extracting document
     * metadata such as titles, headers, links, images, and structured data.
     *
     * <p>
     * <b>Example usage:</b>
     * <pre>{@code
     ...
     *
     * @param html the HTML string to convert
     * @return a {@code MetadataExtraction} containing both markdown and
     *     metadata
     * @throws NullPointerException if html is null
     * @throws ConversionException if the conversion fails
     *
    ...
    public static MetadataExtraction convertWithMetadata(final String html) {
        ...
        try (Arena arena = Arena.ofConfined()) {
            ...
            try {
                String markdown = HtmlToMarkdownFFI.fromCString(
                    resultSegment
                );

                MemorySegment metadataJsonSegment = metadataJsonOut
                    .getAtIndex(
                        java.lang.foreign.ValueLayout.ADDRESS,
                        0
                    );
                ...
                if (metadataJsonSegment != null
                        && metadataJsonSegment.address() != 0) {
                    String metadataJson = HtmlToMarkdownFFI.fromCString(
                        metadataJsonSegment
                    );
                    metadata = parseMetadata(metadataJson);
                    HtmlToMarkdownFFI.html_to_markdown_free_string
                        .invoke(metadataJsonSegment);
                } else {
            ...
            throw new ConversionException(
                "Failed to convert HTML to Markdown with metadata", e
            );
        }
    }
    ...
    private static DocumentMetadata parseDocumentMetadata(
        ...
        String author = node.has("author")
                && !node.get("author").isNull()
            ? node.get("author").asText() : null;
        String canonicalUrl = node.has("canonical_url")
                && !node.get("canonical_url").isNull()
            ? node.get("canonical_url").asText() : null;
        String baseHref = node.has("base_href")
                && !node.get("base_href").isNull()
            ? node.get("base_href").asText() : null;
        String language = node.has("language")
                && !node.get("language").isNull()
            ? node.get("language").asText() : null;
        TextDirection textDirection = null;
        if (node.has("text_direction")
                && !node.get("text_direction").isNull()) {
            String textDirectionStr =
                node.get("text_direction").asText();
            try {
        ...
        if (node.has("open_graph") && node.get("open_graph").isObject()) {
            node.get("open_graph").fields().forEachRemaining(
                e -> openGraph.put(e.getKey(), e.getValue().asText())
            );
        }

        Map<String, String> twitterCard = new TreeMap<>();
        if (node.has("twitter_card")
                && node.get("twitter_card").isObject()) {
            node.get("twitter_card").fields().forEachRemaining(
                e -> twitterCard.put(e.getKey(), e.getValue().asText())
            );
        }

        Map<String, String> metaTags = new TreeMap<>();
        if (node.has("meta_tags") && node.get("meta_tags").isObject()) {
            node.get("meta_tags").fields().forEachRemaining(
                e -> metaTags.put(e.getKey(), e.getValue().asText())
            );
        }
    ...
    private static List<HeaderMetadata> parseHeaders(final JsonNode node) {
        ...
        node.forEach(h -> {
            ...
            String id = h.has("id") && !h.get("id").isNull()
                ? h.get("id").asText() : null;
            int depth = h.has("depth") ? h.get("depth").asInt() : 0;
            int htmlOffset = h.has("html_offset")
                ? h.get("html_offset").asInt() : 0;

            headers.add(new HeaderMetadata(level, text, id, depth,
                htmlOffset));
        });
    ...
    private static List<LinkMetadata> parseLinks(final JsonNode node) {
        ...
        node.forEach(l -> {
            ...
            String title = l.has("title") && !l.get("title").isNull()
                ? l.get("title").asText() : null;
            ...
            if (l.has("attributes")
                    && l.get("attributes").isObject()) {
                l.get("attributes").fields().forEachRemaining(
                    e -> attributes.put(e.getKey(),
                        e.getValue().asText())
                );
            }

            links.add(new LinkMetadata(href, text, title, linkType,
                rel, attributes));
        });
    ...
    private static List<ImageMetadata> parseImages(final JsonNode node) {
        ...
        node.forEach(img -> {
            ...
            String alt = img.has("alt") && !img.get("alt").isNull()
                ? img.get("alt").asText() : null;
            String title = img.has("title")
                    && !img.get("title").isNull()
                ? img.get("title").asText() : null;
            ...
            int[] dimensions = null;
            if (img.has("dimensions") && img.get("dimensions").isArray()
            ...
            Map<String, String> attributes = new HashMap<>();
            if (img.has("attributes")
                    && img.get("attributes").isObject()) {
                img.get("attributes").fields().forEachRemaining(
                    e -> attributes.put(e.getKey(),
                        e.getValue().asText())
                );
            }

            images.add(new ImageMetadata(src, alt, title, dimensions,
     ...
     */
    private static List<StructuredData> parseStructuredData(
            final JsonNode node) {
        ...
        node.forEach(sd -> {
            String dataType = sd.has("data_type")
                ? sd.get("data_type").asText() : "json_ld";
            String rawJson = sd.has("raw_json")
                ? sd.get("raw_json").asText() : "{}";
            String schemaType = sd.has("schema_type")
                    && !sd.get("schema_type").isNull()
                ? sd.get("schema_type").asText() : null;

            structuredData.add(new StructuredData(dataType, rawJson,
                schemaType));
        });
     ...
     * <p>
     * If retrieving the error message fails, logs the exception and
     * returns a descriptive error string.
     *
     * @return the last error message, or a descriptive error string if
     *     retrieval failed
     */
    private static String getLastError() {
        try {
            MemorySegment errorSegment =
                (MemorySegment) HtmlToMarkdownFFI
                    .html_to_markdown_last_error.invoke();
            return HtmlToMarkdownFFI.fromCString(errorSegment);
        } catch (Throwable e) {
            System.err.println(
                "Failed to retrieve FFI error message: "
                    + e.getMessage()
            );
            e.printStackTrace(System.err);
            return "FFI error retrieval failed: "

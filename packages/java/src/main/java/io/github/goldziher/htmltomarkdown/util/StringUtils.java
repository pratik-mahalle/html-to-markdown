...
public final class StringUtils {
    ...
    public static MemorySegment toCString(final Arena arena, final String str) {
        try {
            java.lang.reflect.Method method =
                    Arena.class.getMethod("allocateFrom", String.class);
    ...
    public static String fromCString(final MemorySegment addr) {
        ...
        try {
            java.lang.reflect.Method method =
                    MemorySegment.class.getMethod("getString",
                            long.class);
            return (String) method.invoke(
                    addr.reinterpret(Long.MAX_VALUE), 0L);
        } catch (Exception e) {
            java.util.List<Byte> bytes = new java.util.ArrayList<>();
            int offset = 0;
            while (true) {
                try {
                    byte b = addr.getAtIndex(ValueLayout.JAVA_BYTE,
                            offset);
                    if (b == 0) {
                        break;
                    }

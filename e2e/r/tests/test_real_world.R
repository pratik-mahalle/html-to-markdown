...
test_that("real_world_blog_post: Blog post with headings, paragraphs, code blocks, and links converts to readable Markdown", {
  ...
  expect_true(grepl("# Getting Started with Rust", content, fixed = TRUE))
  expect_true(grepl("## Installation", content, fixed = TRUE))
  expect_true(grepl("## Hello World", content, fixed = TRUE))
  expect_true(grepl("## Key Concepts", content, fixed = TRUE))
  expect_true(grepl("cargo run", content, fixed = TRUE))
  expect_true(grepl("[Mozilla](https://www.mozilla.org)", content, fixed = TRUE))
  expect_true(grepl("- Ownership and borrowing", content, fixed = TRUE))
  expect_true(nchar(content) >= 200)
})
...
test_that("real_world_documentation_page: Documentation page with nested lists, code examples, and blockquotes converts correctly", {
  ...
  expect_true(nchar(trimws(content)) > 0)
  # content_contains_all
  expect_true(grepl("# API Reference", content, fixed = TRUE))
  expect_true(grepl("## convert_html", content, fixed = TRUE))
  expect_true(grepl("### Parameters", content, fixed = TRUE))
  expect_true(grepl("### Returns", content, fixed = TRUE))
  expect_true(grepl("### Example", content, fixed = TRUE))
  expect_true(grepl("## ConversionOptions", content, fixed = TRUE))
  expect_true(grepl("> ", content, fixed = TRUE))
  expect_true(grepl("thread-safe", content, fixed = TRUE))
  expect_true(grepl("convert_html", content, fixed = TRUE))
  expect_true(grepl("ConversionOptions", content, fixed = TRUE))
  expect_true(nchar(content) >= 400)
})
...
test_that("real_world_product_page: Product page with table, images, and lists converts correctly", {
  ...
  expect_true(nchar(trimws(content)) > 0)
  # content_contains_all
  expect_true(grepl("# Wireless Keyboard Pro", content, fixed = TRUE))
  expect_true(grepl("![Wireless Keyboard Pro](https://example.com/keyboard.jpg)", content, fixed = TRUE))
  expect_true(grepl("## Specifications", content, fixed = TRUE))
  expect_true(grepl("Battery Life", content, fixed = TRUE))
  expect_true(grepl("12 months", content, fixed = TRUE))
  expect_true(grepl("Bluetooth 5.0", content, fixed = TRUE))
  expect_true(grepl("## What's in the Box", content, fixed = TRUE))
  expect_true(grepl("USB-C charging cable", content, fixed = TRUE))
  expect_true(grepl("|", content, fixed = TRUE))
  expect_true(grepl("---", content, fixed = TRUE))
  expect_true(nchar(content) >= 300)
})

...
class RealWorldTest extends TestCase
{
     ...
     */
    public function testRealWorldBlogPost(): void
    {
        ...
        $this->assertStringContainsString("# Getting Started with Rust", $content);
        $this->assertStringContainsString("## Installation", $content);
        $this->assertStringContainsString("## Hello World", $content);
        $this->assertStringContainsString("## Key Concepts", $content);
        $this->assertStringContainsString("cargo run", $content);
        $this->assertStringContainsString("[Mozilla](https://www.mozilla.org)", $content);
        $this->assertStringContainsString("- Ownership and borrowing", $content);
        $this->assertGreaterThanOrEqual(200, strlen($content), 'expected content length >= 200');
    }

    /**
     * Documentation page with nested lists, code examples, and blockquotes converts correctly
     */
    public function testRealWorldDocumentationPage(): void
    {
        ...
        $this->assertStringContainsString("## convert_html", $content);
        ...
        $this->assertStringContainsString("## ConversionOptions", $content);
        $this->assertStringContainsString("> ", $content);
        $this->assertStringContainsString("thread-safe", $content);
        $this->assertStringContainsString("convert_html", $content);
        $this->assertStringContainsString("ConversionOptions", $content);
        $this->assertGreaterThanOrEqual(400, strlen($content), 'expected content length >= 400');
    }

    /**
     * Product page with table, images, and lists converts correctly
     */
    public function testRealWorldProductPage(): void
    {
        ...
        $this->assertNotEmpty(trim($content), 'expected non-empty content');
        $this->assertStringContainsString("# Wireless Keyboard Pro", $content);
        $this->assertStringContainsString("![Wireless Keyboard Pro](https://example.com/keyboard.jpg)", $content);
        $this->assertStringContainsString("## Specifications", $content);
        $this->assertStringContainsString("Battery Life", $content);
        $this->assertStringContainsString("12 months", $content);
        $this->assertStringContainsString("Bluetooth 5.0", $content);
        $this->assertStringContainsString("## What's in the Box", $content);
        $this->assertStringContainsString("USB-C charging cable", $content);
        $this->assertStringContainsString("|", $content);
        $this->assertStringContainsString("---", $content);
        $this->assertGreaterThanOrEqual(300, strlen($content), 'expected content length >= 300');
    }

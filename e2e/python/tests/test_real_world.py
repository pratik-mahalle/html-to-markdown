...
def test_real_world_blog_post() -> None:
    ...
    assert "# Getting Started with Rust" in content
    assert "## Installation" in content
    assert "## Hello World" in content
    assert "## Key Concepts" in content
    assert "cargo run" in content
    assert "[Mozilla](https://www.mozilla.org)" in content
    assert "- Ownership and borrowing" in content
    assert len(content) >= 200, f"expected content length >= 200, got {len(content)}"
...
def test_real_world_documentation_page() -> None:
    ...
    assert "## convert_html" in content
    assert "### Parameters" in content
    assert "### Returns" in content
    assert "### Example" in content
    assert "## ConversionOptions" in content
    assert "> " in content
    assert "thread-safe" in content
    assert "convert_html" in content
    assert "ConversionOptions" in content
    assert len(content) >= 400, f"expected content length >= 400, got {len(content)}"
...
def test_real_world_product_page() -> None:
    ...
    assert content.strip() != "", "expected non-empty content"
    assert "# Wireless Keyboard Pro" in content
    assert "![Wireless Keyboard Pro](https://example.com/keyboard.jpg)" in content
    assert "## Specifications" in content
    assert "Battery Life" in content
    assert "12 months" in content
    assert "Bluetooth 5.0" in content
    assert "## What's in the Box" in content
    assert "USB-C charging cable" in content
    assert "|" in content
    assert "---" in content
    assert len(content) >= 300, f"expected content length >= 300, got {len(content)}"

 ...
 *
 * <p>Communicates to the converter how to proceed after visiting a node.
 * Use the factory methods to create appropriate result types.
 *
 * <p><b>Example usage:</b>
 * <pre>{@code
...
public sealed interface VisitResult {
     ...
     *
     * <p>The converter will apply its standard conversion logic and ignore
     * any custom output or error fields.
     *
     * @since 2.17.0
     */
     ...
     *
     * <p>The provided markdown string will be used instead of the
     * converter's default output. The caller is responsible for allocating
     * memory; the converter will free it after use.
     *
    ...
    record Custom(String customOutput) implements VisitResult {
         ...
         */
        public Custom {
            Objects.requireNonNull(customOutput,
                    "Custom output cannot be null");
        }

        @Override
        public String toString() {
            return "VisitResult.Custom[length="
                    + customOutput.length() + "]";
        }
    }
     ...
     *
     * <p>The conversion process will halt immediately, and the error message
     * will be returned to the caller as a ConversionException.
     *

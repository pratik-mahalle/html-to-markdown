...
already_exists() {
	...
	grep -Eq "Component with package url: 'pkg:maven/dev\\.kreuzberg/html-to-markdown@${version}'.*already exists" "$log_file" ||
		grep -Eq "pkg:maven/dev\\.kreuzberg/html-to-markdown@${version}.*already exists" "$log_file"
}
...
if already_exists "$log_file" "$version"; then
	echo "::notice::Maven package dev.kreuzberg:html-to-markdown:${version} already exists; skipping publish." >&2
	exit 0
fi

...
fn render_test_file(category: &str, fixtures: &[&Fixture]) -> String {
    ...
    let _ = writeln!(out);
    let needs_options = fixtures.iter().any(|f| f.options.is_some());
    if needs_options {
        let _ = writeln!(
            out,
            "use html_to_markdown_rs::{{convert, conversion_options_from_json}};"
        );
    } else {
...
fn render_test_function(out: &mut String, fixture: &Fixture) {
    ...
    // Build conversion options if specified.
    if let Some(opts) = &fixture.options {
        ...
        let _ = writeln!(out, "    let options_json = {literal};");
        let _ = writeln!(
            out,
    ...
    let options_arg = if fixture.options.is_some() {
        "Some(options)"
    } else {
        "None"
    };
    ...
    if fixture.assertions.expect_error == Some(true) {
        let _ = writeln!(out, "    let result = convert(html, {options_arg});");
    ...
    let _ = writeln!(
        out,
        "    let result = convert(html, {options_arg}).expect(\"conversion should succeed\");"
    );

...
RSpec.describe 'real-world' do
  it 'real_world_blog_post: Blog post with headings, paragraphs, code blocks, and links converts to readable Markdown' do
    ...
    expect(content).to include('# Getting Started with Rust')
    expect(content).to include('## Installation')
    expect(content).to include('## Hello World')
    expect(content).to include('## Key Concepts')
    expect(content).to include('cargo run')
    expect(content).to include('[Mozilla](https://www.mozilla.org)')
    expect(content).to include('- Ownership and borrowing')
  ...
  it 'real_world_documentation_page: Documentation page with nested lists, code examples, and blockquotes converts correctly' do
    ...
    expect(content).to include('## convert_html')
    expect(content).to include('### Parameters')
    expect(content).to include('### Returns')
    expect(content).to include('### Example')
    expect(content).to include('## ConversionOptions')
    expect(content).to include('> ')
    expect(content).to include('thread-safe')
    expect(content).to include('convert_html')
    expect(content).to include('ConversionOptions')
    expect(content.length).to be >= 400
  end
  ...
  it 'real_world_product_page: Product page with table, images, and lists converts correctly' do
    ...
    expect(content.strip).not_to be_empty
    expect(content).to include('# Wireless Keyboard Pro')
    expect(content).to include('![Wireless Keyboard Pro](https://example.com/keyboard.jpg)')
    expect(content).to include('## Specifications')
    expect(content).to include('Battery Life')
    expect(content).to include('12 months')
    expect(content).to include('Bluetooth 5.0')
    expect(content).to include("## What's in the Box")
    expect(content).to include('USB-C charging cable')
    expect(content).to include('|')
    expect(content).to include('---')
    expect(content.length).to be >= 300
  end
end

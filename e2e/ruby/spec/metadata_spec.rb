...
RSpec.describe 'metadata' do
  ...
  it 'structured_data_json_ld: JSON-LD script tag is stripped from output (security) but metadata may be extracted' do
    ...
    expect(content.strip).not_to be_empty
    expect(content).to include("My Article")
    expect(content).not_to include("application/ld+json")
    expect(content).not_to include("@context")
    expect(content).not_to include("schema.org")
  end
  ...
  it 'structured_data_multiple_json_ld: Multiple JSON-LD blocks are all stripped from output' do
    ...
    expect(content).not_to include("@context")
    expect(content).not_to include("BreadcrumbList")
    expect(content).not_to include("ListItem")
  end

...
def test_emphasis_strikethrough_del() -> None:
    ...
    assert "~~deleted text~~" in content
...
def test_image_figure_figcaption() -> None:
    ...
    assert "![A sunset](sunset.jpg)" in content
    assert "Beautiful sunset over the ocean" in content
...
def test_line_break_hr_tag() -> None:
    ...
    assert content.strip() != "", "expected non-empty content"
    assert "Before rule." in content
    assert "After rule." in content
...
def test_link_anchor_fragment() -> None:
    ...
    assert "[Jump to section](#section)" in content
...
def test_link_empty_href() -> None:
    ...
    assert "No destination" in content
...
def test_link_image_inside() -> None:
    ...
    assert "![Logo](logo.png)" in content
    assert "https://example.com" in content
...
def test_list_definition_dl() -> None:
    ...
    assert "Term One" in content
    assert "Definition of term one." in content
    assert "Term Two" in content
    assert "Definition of term two." in content
...
def test_list_item_multiple_paragraphs() -> None:
    ...
    assert "First paragraph in item." in content
    assert "Second paragraph in item." in content
...
def test_list_nested_unordered() -> None:
    ...
    assert "Child A1" in content
    assert "Child A2" in content
...
def test_semantic_definition_list() -> None:
    ...
    assert "Cascading Style Sheets" in content

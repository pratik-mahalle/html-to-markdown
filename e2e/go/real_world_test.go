...
func Test_RealWorldBlogPost(t *testing.T) {
	...
	if !strings.Contains(content, "# Getting Started with Rust") {
		t.Errorf("expected content to contain: # Getting Started with Rust")
	}
	if !strings.Contains(content, "## Installation") {
		t.Errorf("expected content to contain: ## Installation")
	}
	if !strings.Contains(content, "## Hello World") {
		t.Errorf("expected content to contain: ## Hello World")
	}
	if !strings.Contains(content, "## Key Concepts") {
		t.Errorf("expected content to contain: ## Key Concepts")
	}
	if !strings.Contains(content, "cargo run") {
		t.Errorf("expected content to contain: cargo run")
	}
	if !strings.Contains(content, "[Mozilla](https://www.mozilla.org)") {
		t.Errorf("expected content to contain: [Mozilla](https://www.mozilla.org)")
	}
	if !strings.Contains(content, "- Ownership and borrowing") {
		t.Errorf("expected content to contain: - Ownership and borrowing")
	}
	if len(content) < 200 {
		t.Errorf("expected content length >= 200, got %d", len(content))
	}
}
...
assert_eq!(markdown, \"# Hello\");</code></pre><h2>ConversionOptions</h2><p>Configure conversion behavior using the builder pattern:</p><pre><code class=\"language-rust\">let options = ConversionOptions::builder()
	...
	if !strings.Contains(content, "# API Reference") {
		t.Errorf("expected content to contain: # API Reference")
	}
	if !strings.Contains(content, "## convert_html") {
		t.Errorf("expected content to contain: ## convert_html")
	}
	if !strings.Contains(content, "### Parameters") {
		t.Errorf("expected content to contain: ### Parameters")
	}
	if !strings.Contains(content, "### Returns") {
		t.Errorf("expected content to contain: ### Returns")
	}
	if !strings.Contains(content, "### Example") {
		t.Errorf("expected content to contain: ### Example")
	}
	if !strings.Contains(content, "## ConversionOptions") {
		t.Errorf("expected content to contain: ## ConversionOptions")
	}
	if !strings.Contains(content, "> ") {
		t.Errorf("expected content to contain: > ")
	}
	if !strings.Contains(content, "thread-safe") {
		t.Errorf("expected content to contain: thread-safe")
	}
	if !strings.Contains(content, "convert_html") {
		t.Errorf("expected content to contain: convert_html")
	}
	...
	if len(content) < 400 {
		t.Errorf("expected content length >= 400, got %d", len(content))
	}
}
...
func Test_RealWorldProductPage(t *testing.T) {
	...
	if !strings.Contains(content, "# Wireless Keyboard Pro") {
		t.Errorf("expected content to contain: # Wireless Keyboard Pro")
	}
	if !strings.Contains(content, "![Wireless Keyboard Pro](https://example.com/keyboard.jpg)") {
		t.Errorf("expected content to contain: ![Wireless Keyboard Pro](https://example.com/keyboard.jpg)")
	}
	if !strings.Contains(content, "## Specifications") {
		t.Errorf("expected content to contain: ## Specifications")
	}
	if !strings.Contains(content, "Battery Life") {
		t.Errorf("expected content to contain: Battery Life")
	}
	if !strings.Contains(content, "12 months") {
		t.Errorf("expected content to contain: 12 months")
	}
	if !strings.Contains(content, "Bluetooth 5.0") {
		t.Errorf("expected content to contain: Bluetooth 5.0")
	}
	if !strings.Contains(content, "## What's in the Box") {
		t.Errorf("expected content to contain: ## What's in the Box")
	}
	if !strings.Contains(content, "USB-C charging cable") {
		t.Errorf("expected content to contain: USB-C charging cable")
	}
	if !strings.Contains(content, "|") {
		t.Errorf("expected content to contain: |")
	}
	...
	if len(content) < 300 {
		t.Errorf("expected content length >= 300, got %d", len(content))
	}
}

...
EXT_FILE="libhtml_to_markdown_php.${EXT_SUFFIX}"
...
if [[ -d "${TARGET_DIR}" ]]; then
  ...
  shopt -s nullglob
  files=("${TARGET_DIR}"/*html_to_markdown* "${TARGET_DIR}"/*.dll "${TARGET_DIR}"/*.so "${TARGET_DIR}"/*.dylib)
  if [[ ${#files[@]} -gt 0 ]]; then
    ls -la "${files[@]}"
  else
    echo "No html_to_markdown binaries found"
  fi
else
...
if [[ ! -f "${TARGET_DIR}/${EXT_FILE}" ]]; then
  ...
  for ext in .so .dylib; do
    for prefix in libhtml_to_markdown_php html_to_markdown_php; do
...
PKG_NAME="html-to-markdown-${VERSION}-${PLATFORM}"
...
This is a pre-built PIE package for the html-to-markdown PHP extension.
...
\`\`\`bash
php -m | grep html_to_markdown
\`\`\`

## Support

For issues, visit: https://github.com/kreuzberg-dev/html-to-markdown/issues
EOF

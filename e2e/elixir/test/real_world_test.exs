...
defmodule HtmlToMarkdown.RealWorldTest do
  use ExUnit.Case

  test "real_world_blog_post: Blog post with headings, paragraphs, code blocks, and links converts to readable Markdown" do
    html =
    ...
    assert String.contains?(content, "\# Getting Started with Rust")
    assert String.contains?(content, "\#\# Installation")
    assert String.contains?(content, "\#\# Hello World")
    assert String.contains?(content, "\#\# Key Concepts")
    assert String.contains?(content, "cargo run")
    assert String.contains?(content, "[Mozilla](https://www.mozilla.org)")
    assert String.contains?(content, "- Ownership and borrowing")
    assert String.length(content) >= 200
  end

  test "real_world_documentation_page: Documentation page with nested lists, code examples, and blockquotes converts correctly" do
    html =
    ...
    assert String.trim(content) != ""
    assert String.contains?(content, "\# API Reference")
    assert String.contains?(content, "\#\# convert_html")
    assert String.contains?(content, "\#\#\# Parameters")
    assert String.contains?(content, "\#\#\# Returns")
    assert String.contains?(content, "\#\#\# Example")
    assert String.contains?(content, "\#\# ConversionOptions")
    assert String.contains?(content, "> ")
    assert String.contains?(content, "thread-safe")
    assert String.contains?(content, "convert_html")
    assert String.contains?(content, "ConversionOptions")
    assert String.length(content) >= 400
  end

  test "real_world_product_page: Product page with table, images, and lists converts correctly" do
    html =
    ...
    assert String.trim(content) != ""
    assert String.contains?(content, "\# Wireless Keyboard Pro")
    assert String.contains?(content, "![Wireless Keyboard Pro](https://example.com/keyboard.jpg)")
    assert String.contains?(content, "\#\# Specifications")
    assert String.contains?(content, "Battery Life")
    assert String.contains?(content, "12 months")
    assert String.contains?(content, "Bluetooth 5.0")
    assert String.contains?(content, "\#\# What's in the Box")
    assert String.contains?(content, "USB-C charging cable")
    assert String.contains?(content, "|")
    assert String.contains?(content, "---")

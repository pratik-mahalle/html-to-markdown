...
// alef:hash:d11351752ab939aa732cbdc5c7b025554349e5d81483188a809ee6b63ca239de
...
impl html_to_markdown_rs::visitor::HtmlVisitor for RbHtmlVisitorBridge {
    ...
    fn visit_link(
        ...
        let result: Result<magnus::Value, magnus::Error> = self.rb_obj.funcall(
            ...
            (
                ...
                match _title {
                    Some(s) => magnus::RString::new(s).as_value(),
    ...
    fn visit_list_end(
        ...
        let result: Result<magnus::Value, magnus::Error> = self.rb_obj.funcall(
            "visit_list_end",
            (nodecontext_to_rb_hash(_ctx), _ordered, magnus::RString::new(_output)),
        );
        match result {
            Err(_) => html_to_markdown_rs::VisitResult::Continue,
            Ok(val) => {
    ...
    fn visit_blockquote(
        ...
        let result: Result<magnus::Value, magnus::Error> = self.rb_obj.funcall(
            "visit_blockquote",
            (nodecontext_to_rb_hash(_ctx), magnus::RString::new(_content), _depth),
        );
        match result {
            Err(_) => html_to_markdown_rs::VisitResult::Continue,
            Ok(val) => {
...
impl From<html_to_markdown_rs::metadata::ImageMetadata> for ImageMetadata {
    fn from(val: html_to_markdown_rs::metadata::ImageMetadata) -> Self {
        Self {
            ...
            alt: val.alt,
            title: val.title,
            dimensions: val.dimensions.clone(),
            image_type: val.image_type.into(),
            attributes: val.attributes.into_iter().collect(),
        }
    }

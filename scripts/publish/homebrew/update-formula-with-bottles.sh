...
formula_path="$tap_dir/Formula/html-to-markdown.rb"
...
cd "$tap_dir"
git config user.name "html-to-markdown-bot"
git config user.email "bot@kreuzberg.dev"

if git diff --quiet Formula/html-to-markdown.rb; then
	echo "No changes to formula"
	exit 0
fi

git add Formula/html-to-markdown.rb
git commit -m "chore(homebrew): update html-to-markdown to $version
